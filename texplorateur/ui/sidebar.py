...
class Sidebar(ctk.CTkFrame):
    ...
    def __init__(self, parent, app, on_navigate, on_nouvelle_recherche):
        ...
        # Logo réel (image.ico) + nom de l'app dans un seul label. Un aparté
        # historique : ce label combinait avant un emoji et le texte dans
        # une police sans glyphe emoji, ce qui corrompait silencieusement le
        # rendu d'autres polices emoji créées plus tard dans le process ;
        # utiliser une vraie image évite ce problème par construction.
        self.label_titre = ctk.CTkLabel(
            self, image=image_icone(20), text=" Texplorateur", compound="left", font=font_titre(16))
